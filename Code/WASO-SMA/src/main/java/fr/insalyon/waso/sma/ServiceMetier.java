...
public class ServiceMetier {
    ...
    public void rechercherClientParNumero(int numero) throws ServiceException {
        try {
            ...
            clientContainerElement = this.jsonHttpClient.post(this.somClientUrl, new BasicNameValuePair("SOM", "rechercherClientParNumero"), new BasicNameValuePair("numero", String.valueOf(numero)));
            System.err.println(clientContainerElement);
            if (clientContainerElement == null) {
    ...
    public void rechercherClientParNomPersonne(String nomPersonne, String ville) throws ServiceException {
        try {
            ...
            // 1. Obtenir la liste des idsPersonnes
            
            JsonElement personneIdsContainerElement = null;

            personneIdsContainerElement = this.jsonHttpClient.post(this.somPersonneUrl, new BasicNameValuePair("SOM", "rechercherPersonneParNom"), new BasicNameValuePair("nom-personne", nomPersonne), new BasicNameValuePair("ville", ville));

            if (personneIdsContainerElement == null) {
                throw new ServiceException("Appel impossible au Service Personne::rechercherPersonneParNom [" + this.somPersonneUrl + "]");
            }
            
            String idsPersonne = "";
            System.out.println(personneIdsContainerElement.getAsJsonObject().getAsJsonArray("personnes"));
            for (JsonElement personne : personneIdsContainerElement.getAsJsonObject().getAsJsonArray("personnes")){
                JsonObject p = personne.getAsJsonObject();
                idsPersonne += String.valueOf(p.get("id").getAsInt()) + ",";
            }
            idsPersonne = idsPersonne.substring(0, idsPersonne.length()-1);

            if(idsPersonne.equals("")) {
                throw new ServiceException("Aucun id");
            }
            ...
            clientContainerElement = this.jsonHttpClient.post(this.somClientUrl, new BasicNameValuePair("SOM", "rechercherClientParPersonne"), new BasicNameValuePair("idsPersonne", idsPersonne), new BasicNameValuePair("ville", ville));
            ...
            // 3. Obtenir la liste des Personnes
