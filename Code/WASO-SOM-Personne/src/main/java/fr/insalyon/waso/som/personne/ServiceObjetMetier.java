...
public class ServiceObjetMetier {
    ...
    public void rechercherPersonneParNom(String nomPersonne) throws ServiceException {
        try {
            List<Object[]> listePersonne = this.dBConnection.launchQuery("SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE WHERE NOM LIKE '%"+nomPersonne+"%' ORDER BY PersonneID");
